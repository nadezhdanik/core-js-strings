...
function removeFirstOccurrences(str, value) {
  ...
  const secondHalf = str.slice(index + value.length);
...
function isPalindrome(str) {
  const etalon = str
    ...
    .replaceAll('?', '')
    .toLowerCase();
  const reversed = etalon.split('').reverse().join('');
  return etalon === reversed;
}
...
function encodeToRot13(str) {
  const alphabetDoubled =
    'ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz';
  return str
    .split('')
    .map((el) => {
      if (!alphabetDoubled.includes(el)) {
        return el;
      }
      const index = alphabetDoubled.indexOf(el);
      return alphabetDoubled[index + 13];
    })
    .join('');
}
